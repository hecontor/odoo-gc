...
class ResPartner(models.Model):
    ...
    # )
    borrowed_loan_ids = fields.One2many("account.loan", inverse_name="partner_id")  # era lended_loan_ids
    borrowed_loan_count = fields.Integer(
        compute="_compute_borrowed_loan_count",
        help="How many loans has this partner borrowed from us?",
    )

    @api.depends("borrowed_loan_ids")
    def _compute_borrowed_loan_count(self):
        for record in self:
            record.borrowed_loan_count = len(record.borrowed_loan_ids)
